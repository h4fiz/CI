...
class Mdl_hukuman_disiplin extends CI_Model
{
    ...
    {
        $query = $this->db->query("select count(*) as jml from riwayat_hukuman where NIP = '$NIP_baru' ");
    ...
    public function fetch_data_keluarga($NIP_baru, $limit, $start) 
    {
        ...
        if($start==0)
        {
            $query = $this->db->query("SELECT * FROM (select * from riwayat_hukuman where NIP = '$NIP_baru') a LIMIT $limit");
        } 
        else
        {   
            $query = $this->db->query("SELECT * FROM (select * from riwayat_hukuman where NIP = '$NIP_baru') a LIMIT $start,$limit");
        }
    ...
    function _delete($id)
    {
        $this->db->delete('riwayat_hukuman', array('Id_Rhk' => $id)); 
    }
}