...
class Data_diklat extends MX_Controller  {

    public function __construct() 
    {
        parent:: __construct();
        $this->load->helper("url");
        $this->load->model("mdl_data_diklat");
...
/*			$data['query_agama'] = $this->mdl_data_profil_pegawai->get_agama('Kd_Agama');
	        ...
	        $config = array();
	        $config["base_url"] = base_url() . "data_diklat/daftar";
	        $config["total_rows"] = $this->mdl_data_diklat->record_count();
	        $config["per_page"] = 10;
	        $config["uri_segment"] = 3;
	        ...
	        $page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
	        $data["results"] = $this->mdl_data_diklat->
	            fetch_data_profil_pegawai($config["per_page"], $page);
	        $data["links"] = $this->pagination->create_links();
	...
	public function detail()
	{
		...
		if($data['username']!="" && $data['password']!="")
		{
			...
			$data['Nama'] = $this->mdl_data_diklat->get_name_pegawai($data['NIP']);
			$data['title'] = "Data Diklat ".$data['Nama'];
			$data['title_header'] = "Data Diklat";
			$data['view_file'] = "index_detail";
			$data['modules'] = "data_diklat";
			$data['javascript'] = "data_diklat.js";

			$data['jml_row'] = $this->mdl_data_diklat->record_count_diklat($data['NIP']);


	        $config = array();
	        $config["base_url"] = base_url() . "data_diklat/detail/".$data['NIP']."/";
	        $config["total_rows"] = $this->mdl_data_diklat->record_count_diklat($data['NIP']);
	        $config["per_page"] = 10;
	...
	public function simpan()
	{
		...
		$this->mdl_data_diklat->_insert($data_insert);
		redirect(base_url() . "data_diklat/detail/".$data['NIP']."/");
	...
	public function ubah()
	{
		...
		$this->mdl_data_diklat-> _update($id,$data_update);
		redirect(base_url() . "data_diklat/detail/".$data['NIP']."/");
		//redirect(base_url().'data_diklat/daftar_utama');
	}
	...
	public function hapus()
	{
		...
		$this->mdl_data_diklat->_delete($id);
		redirect(base_url() . "data_diklat/detail/".$data['NIP']."/");
		//redirect(base_url().'data_diklat/daftar_utama');
	}
}